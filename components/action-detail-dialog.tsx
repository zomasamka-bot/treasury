...
import { getStatusColor, RELEASE_POLICY_TEMPLATES } from "@/lib/treasury-types";
import { CheckCircle2, Clock, FileText, XCircle, Shield, Link2, Zap } from "lucide-react";
...
export function ActionDetailDialog({ action, open, onOpenChange }: ActionDetailDialogProps) {
  ...
  // Get the release policy template for this reserve type (informational only)
  const releasePolicy = RELEASE_POLICY_TEMPLATES.find(p => p.type === action.type);
                ...
                <div>
                  ...
                  <div className="bg-accent/5 border border-accent/10 rounded-lg p-3 space-y-2">
                    <p className="text-xs text-muted-foreground leading-relaxed">
                      {releasePolicy.description}
                    </p>
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-muted-foreground">{'Release Mode'}</span>
                      <Badge variant="outline" className="text-xs capitalize">
                        {releasePolicy.releaseMode.replace('-', ' ')}
                      </Badge>
                    </div>
                  </div>
                </div>
